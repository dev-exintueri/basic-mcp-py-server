...
**파이썬 구현이 기준이다.** 동작이 갈리면 파이썬이 맞는 쪽이고, 노드를
고친다. 두 서버가 실제로 같은지는 `plugins/mcp-test/conformance/`가
강제하지만, 그 스위트가 계약하는 범위는 [노드 서버 설계
문서](docs/superpowers/specs/2026-07-26-nodejs-server-design.md) §3.1 이고
**계약하지 않는 것은 §3.2 에 적혀 있다.** 파이썬 쪽을 바꿨다면 §3.1 표를
보고 노드에도 반영한다.
